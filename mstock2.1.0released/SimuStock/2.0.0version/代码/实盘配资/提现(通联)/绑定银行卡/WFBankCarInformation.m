...
- (void)awakeFromNib {
  ...
  //选择银行卡按钮的高亮颜色
  [self.actionButton setBackgroundImage:[SimuUtil imageFromColor:@"#131313"]
                               forState:UIControlStateHighlighted];
}

- (IBAction)buttonAction:(UIButton *)sender {
  WFSearchBankViewController *searchVC =
      [[WFSearchBankViewController alloc] init];
  [AppDelegate pushViewControllerFromRight:searchVC];
  [searchVC bankWithCallback:^(NSString *bankName, NSString *bankID) {
    ...
    NSLog(@"银行名称 =  %@ ,银行ID = %@", bankName, bankID);
  }];
}
- (void)textFieldDidBeginEditing:(UITextField *)textField {
  if (_delegate &&
      [_delegate respondsToSelector:@selector(ChangeScrollviewHight)]) {
