//
//  TrendChatStockPageTableVC.m
//  SimuStock
//
//  Created by jhss_wyz on 15/7/24.
...
@implementation TrendChatStockPageTVAdapter
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  cellHeight += [TrendChatStockPageTVAdapter getTitleAndContentAndImageHeightWithWeibo:item
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  /// 绑定聊股标题、聊股内容、聊股图片数据
  height += [TrendChatStockPageTVAdapter bindTittleAndContentAndContentImageAtCell:cell
                                                                      andIndexPath:indexPath
                                                                  andTopViewBottom:height
                                                                      andTableView:tableView
                                                                andHasUserNameView:NO];

  height += [TrendChatStockPageTVAdapter bindRPContentAndRPComtentImageAtCell:cell
                                                                 andIndexPath:indexPath
                                                                 andTableView:tableView
                                                             andTopViewBottom:height
                                                           andHasUserNameView:NO];
  ...
  __weak TrendChatStockPageTVAdapter *weakSelf = self;
  /// 重写取消收藏回调函数，屏蔽默认实现
...
@implementation TrendChatStockPageTableVC
...
- (NSDictionary *)getRequestParamertsWithRefreshType:(RefreshType)refreshType {
  ...
  return @{ @"stockCode" : stockCode, @"fromId" : [fromId stringValue], @"reqNum" : pageSize };
}
...
- (void)requestDataListWithRefreshType:(RefreshType)refreshType
  ...
  [TweetList requestGetStockTopicTweetListWithParams:dic withCallback:callback];
}

/** 返回表格数据适配器，此方法子类必须实现 */
- (BaseTableAdapter *)getBaseTableAdaperWithDataList:(DataArray *)dataList {
  if (_tableAdapter == nil) {
    _tableAdapter = [[TrendChatStockPageTVAdapter alloc] initWithTableViewController:self
    ...
    TrendChatStockPageTVAdapter *adapter = (TrendChatStockPageTVAdapter *)_tableAdapter;
