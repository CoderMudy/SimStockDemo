...
@implementation MyChatStockTVCell
...
- (void)awakeFromNib {
  ...
  if (self.bottomToolView) {
    ...
    NSArray *forbitLongPressedViews = @[ self.shareBtn, self.commentBtn, self.applaudBtn ];
    for (UIView *view in forbitLongPressedViews) {
      UILongPressGestureRecognizer *shieldLongPress =
          [[UILongPressGestureRecognizer alloc] initWithTarget:view action:nil];
      shieldLongPress.minimumPressDuration = LongPressTime;
      [view addGestureRecognizer:shieldLongPress];
    }
  }
}
...
- (void)cellClickAction:(UILongPressGestureRecognizer *)gesture {
  ...
  if (gesture.state == UIGestureRecognizerStateBegan) {
    ...
    if ([SimuUtil isLogined]) {
      ...
      /// 显示拓展按钮
      if (self.bottomToolView) {
        [WeiBoExtendButtons showWithTweetListItem:self.tweetListItem offsetY:loc.y cell:self];
      } else {
...
- (void)setExtendButtonsBlock {
  ...
  [extendButtons setExtendDeleteButtonClickBlock:^(NSNumber *tid, NSObject *cell) {
    MyChatStockTVCell *strongCell = (MyChatStockTVCell *)cell;
  ...
  /// 取消收藏按钮
  [extendButtons setExtendCancleCollectButtonClickBlock:^(NSNumber *tid, NSObject *cell) {
    MyChatStockTVCell *strongCell = (MyChatStockTVCell *)cell;
    if (strongCell.cancleCollectBtnBlock) {
      strongCell.cancleCollectBtnBlock(tid);
    }
  }];
}

- (void)bidButtonTriggeringMethod:(UIButton *)btn {
  if ([self.delegate respondsToSelector:@selector(bidButtonTriggersCallbackMethod:row:)]) {
...
- (void)resetBottomToolBtnTitle {
  ...
  if (self.tweetListItem.comment > 0) {
    [self.commentBtn setTitle:[NSString stringWithFormat:@"%ld", (long)_tweetListItem.comment]
                     forState:UIControlStateNormal];
  } else {
  ...
  if (_tweetListItem.praise > 0) {
    [self.applaudBtn setTitle:[NSString stringWithFormat:@"%ld", (long)_tweetListItem.praise]
                     forState:UIControlStateNormal];
  } else {
    [self.applaudBtn setTitle:@"赞" forState:UIControlStateNormal];
  }
  /// 是否点过赞
  self.tweetListItem.isPraised = [WBCoreDataUtil fetchPraiseTid:@([_tweetListItem.tstockid longLongValue])];
  if (_tweetListItem.isPraised) {
    self.applaudBtn.userInteractionEnabled = NO;
    [self.applaudBtn setTitle:[NSString stringWithFormat:@"%ld", (long)_tweetListItem.praise]
                     forState:UIControlStateNormal];
    [self.applaudBtn setImage:[UIImage imageNamed:@"赞小图标_down"] forState:UIControlStateNormal];
    //设置点赞数字体颜色(红色)
  ...
  [self.commentBtn setImage:[UIImage imageNamed:@"评论小图标1"] forState:UIControlStateNormal];
}
