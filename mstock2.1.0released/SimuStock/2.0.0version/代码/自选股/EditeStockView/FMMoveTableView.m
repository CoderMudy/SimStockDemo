...
/**
 * When the long press gesture recognizer began, we create a snap shot of the
 * touched
 * cell so the user thinks that he is moving the cell itself. Instead we clear
 * out the
...
/**
 * We need a little helper to cancel the current touch of the long press gesture
 * recognizer
...
- (void)moveRowToLocation:(CGPoint)location {
  ...
  if ([newIndexPath section] != NSNotFound &&
      ...
      newIndexPath ==
          [[self delegate] moveTableView:self
              targetIndexPathForMoveFromRowAtIndexPath:[self movingIndexPath]
                                   toProposedIndexPath:newIndexPath]) {
    [self beginUpdates];

    // Move the row
    [self
        deleteRowsAtIndexPaths:@[[self movingIndexPath]]
              withRowAnimation:UITableViewRowAnimationAutomatic];
    [self insertRowsAtIndexPaths:@[newIndexPath]
                withRowAnimation:UITableViewRowAnimationAutomatic];

    // Inform the delegate to update it's model
    if ([[self dataSource] respondsToSelector:@selector(moveTableView:
                                                  moveRowFromIndexPath:
                                                           toIndexPath:)]) {
      [[self dataSource] moveTableView:self
                  moveRowFromIndexPath:[self movingIndexPath]
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer {
  ...
  case UIGestureRecognizerStateBegan: {
    ...
    if (!touchedIndexPath || [touchedIndexPath section] == NSNotFound ||
        [touchedIndexPath row] == NSNotFound) {
    ...
    FMMoveTableViewCell *touchedCell =
        (FMMoveTableViewCell *)[self cellForRowAtIndexPath:touchedIndexPath];
    [touchedCell setSelected:NO];
    [touchedCell setHighlighted:NO];

    
    // Compute the touch offset from the cell's center
    CGPoint touchOffset = CGPointMake([touchedCell center].x - touchPoint.x,
                                      [touchedCell center].y - touchPoint.y);
    ...
    FMSnapShotImageView *snapShotOfMovingCell =
        [[FMSnapShotImageView alloc] initWithImage:image];
    ...
    if ([[self delegate] respondsToSelector:@selector(moveTableView:
                                                willMoveRowAtIndexPath:)]) {
      [[self delegate] moveTableView:self
              willMoveRowAtIndexPath:touchedIndexPath];
    }

    // Set a threshold for autoscrolling and reset the autoscroll distance
    [self setAutoscrollThreshold:([[self snapShotImageView] frame].size.height *
                                  0.6)];
  ...
  case UIGestureRecognizerStateChanged: {
    ...
    [[self snapShotImageView]
        setCenter:CGPointMake(currentCenter.x,
    ...
    // If the table view does not scroll, compute a new index path for the
    // moving cell
    ...
    [UIView animateWithDuration:0.2
        ...
        completion:^(BOOL finished) {
            ...
            if (finished) {
              ...
              [self reloadRowsAtIndexPaths:@[movingIndexPath]
...
- (void)maybeAutoscrollForSnapShotImageView:(FMSnapShotImageView *)snapShot {
  ...
  if (CGRectIntersectsRect([snapShot frame], [self bounds])) {
    CGPoint touchLocation =
        [[self movingGestureRecognizer] locationInView:self];
    ...
    if (distanceToTopEdge < [self autoscrollThreshold]) {
      [self setAutoscrollDistance:
                [self autoscrollDistanceForProximityToEdge:distanceToTopEdge] *
                -1];
    } else if (distanceToBottomEdge < [self autoscrollThreshold]) {
      [self setAutoscrollDistance:[self autoscrollDistanceForProximityToEdge:
...
- (void)legalizeAutoscrollDistance {
  ...
  float maximumLegalDistance =
      [self contentSize].height -
      ([self frame].size.height + [self contentOffset].y);
  [self setAutoscrollDistance:MAX([self autoscrollDistance],
                                  minimumLegalDistance)];
  [self setAutoscrollDistance:MIN([self autoscrollDistance],
                                  maximumLegalDistance)];
}
...
- (void)setSnapShotImageView:(FMSnapShotImageView *)snapShotImageView {
  if (snapShotImageView) {
    ...
    CGPathRef shadowPath = [[UIBezierPath
        bezierPathWithRect:[[snapShotImageView layer] bounds]] CGPath];
