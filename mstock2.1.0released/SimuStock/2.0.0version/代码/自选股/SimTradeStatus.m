...
#import "SimTradeStatus.h"

@implementation SimTradeStatus {
  ///是否正在请求倒计时数据
  BOOL isRequesting;

  ///距开市、闭市的倒计时时间，单位：秒
  long long int countDownSeconds;
}
...
+ (SimTradeStatus *)instance {
  static SimTradeStatus *instance = nil;
  static dispatch_once_t predicate;
  dispatch_once(&predicate, ^{
      instance = [[SimTradeStatus alloc] init];

      NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
      NSDictionary *dic = [userDefaults objectForKey:@"SimTradeStatus"];

      if (dic) {
        long long ctime = [[NSDate date] timeIntervalSince1970];
        long long mtime = [dic[@"mtime"] longLongValue];
...
- (TradeStatus)closedStatus:(long long)countDownMilliSeconds {
...
- (NSString *)formatTimeBySeconds:(long long)time_seconds {
  //秒钟换算成时间
...
- (void)requestExchangeStatus:(void(^)())onRefreshCallBack {
  ...
  callback.onSuccess = ^(NSObject *obj) {
      isRequesting = NO;
      SimTradeStatus *status = (SimTradeStatus *)obj;
      SimTradeStatus *instance = [SimTradeStatus instance];
  ...
  [requester
      asynExecuteWithRequestUrl:
         ...
         withRequestObjectClass:[SimTradeStatus class]
