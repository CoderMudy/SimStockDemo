...
+ (void)setMessageContent:(NSString *)message {
  ...
  newShowLab.numberOfLines = 0;
  newShowLab.backgroundColor =
      [Globle colorFromHexRGB:Color_Blue_but alpha:0.8];
  ...
  [arrayWindows enumerateObjectsUsingBlock:^(UIWindow * obj, NSUInteger idx, BOOL *stop) {
    if (obj.windowLevel != 1001) {
      [resultArray addObject:obj];
    }
    
  }];
  if (SYSTEM_VERSION >= 7.0f) {
    window = resultArray.lastObject;
  } else {
    window = resultArray.lastObject;
