...
- (void)setAction:(ButtonPressed)action {
  objc_setAssociatedObject(self, &overviewKeyBlock, action, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setLastPressedTime:(NSNumber *)lastPressedTime {
  objc_setAssociatedObject(self, &overviewKeyTime, lastPressedTime, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setHighlightTitleColor:(UIColor *)highlightTitleColor {
  if (highlightTitleColor) {
    ...
    [self setTitleColor:_highlightTitleColor forState:UIControlStateHighlighted];
  }
}
/** 设置button title */
