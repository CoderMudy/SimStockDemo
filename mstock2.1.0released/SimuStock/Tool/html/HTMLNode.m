...
NSString *getAttributeNamed(xmlNode *node, const char *nameStr) {
  for (xmlAttrPtr attr = node->properties; NULL != attr; attr = attr->next) {
    if (strcmp((char *)attr->name, nameStr) == 0) {
      for (xmlNode *child = attr->children; NULL != child;
           child = child->next) {
        return [NSString stringWithCString:(void *)child->content
...
- (NSString *)tagName {
  return [NSString stringWithCString:(void *)_node->name
                            encoding:NSUTF8StringEncoding];
}
        ...
        for (xmlNode *child = attr->children; NULL != child;
          ...
          if (!partial && strcmp((char *)child->content, classNameStr) == 0)
            match = YES;
                   ...
                   strstr((char *)child->content, classNameStr) != NULL)
            match = YES;
...
- (HTMLNode *)findChildTag:(NSString *)tagName inXMLNode:(xmlNode *)node {
  ...
  for (cur_node = node; cur_node; cur_node = cur_node->next) {
    if (cur_node && cur_node->name &&
        strcmp((char *)cur_node->name, tagNameStr) == 0) {
...
[self contents]];
        ...
        string = [string stringByAppendingString:[NSString
stringWithFormat:@"<%s>\n", _node->name]];
...
- (HTMLNode *)findChildWithAttribute:(const char *)attribute
  ...
  for (cur_node = node; cur_node; cur_node = cur_node->next) {
    ...
    HTMLNode *cNode = [self findChildWithAttribute:attribute
                                      matchingName:name
                                         inXMLNode:cur_node->children
                                      allowPartial:partial];
    if (cNode != NULL) {
...
- (HTMLNode *)findChildWithAttribute:(NSString *)attribute
  ...
  return [self findChildWithAttribute:[attribute UTF8String]
                         matchingName:[className UTF8String]
                            inXMLNode:_node->children
                         allowPartial:partial];
}

- (HTMLNode *)findChildOfClass:(NSString *)className {
  HTMLNode *node = [self findChildWithAttribute:"class"
                                   matchingName:[className UTF8String]
                                      inXMLNode:_node->children
                                   allowPartial:NO];
  return node;
...
- (NSArray *)findChildrenOfClass:(NSString *)className {
  return [self findChildrenWithAttribute:@"class"
                            matchingName:className
                            allowPartial:NO];
}
...
- (void)appendChildContentsToString:(NSMutableString *)string
  ...
  for (cur_node = node; cur_node; cur_node = cur_node->next) {
    if (cur_node->content) {
      [string appendString:[NSString stringWithCString:(void *)cur_node->content
                                              encoding:NSUTF8StringEncoding]];
    }
...
- (NSString *)contents {
  if (_node->children && _node->children->content) {
    return [NSString stringWithCString:(void *)_node->children->content
                              encoding:NSUTF8StringEncoding];
  }
...
NSString *allNodeContents(xmlNode *node) {
  ...
  if (contents) {
    ...
    NSString *string =
        [NSString stringWithCString:contents encoding:NSUTF8StringEncoding];
    xmlFree(contents);
    return string;
  }
...
NSString *rawContentsOfNode(xmlNode *node) {
  ...
  if (buffer->content) {
    string =
        [[NSString alloc] initWithBytes:(const void *)xmlBufferContent(buffer)
