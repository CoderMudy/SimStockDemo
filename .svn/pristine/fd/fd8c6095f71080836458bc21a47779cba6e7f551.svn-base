...
- (NSInteger)indexOfString:(NSString *)s {
  NSAssert3((s != nil), @"Error, s is a nil string, %s, %s, %d", __FILE__,
...
- (NSInteger)indexOfString:(NSString *)s fromIndex:(NSInteger)index {
  ...
  NSRange searchRange =
      NSMakeRange((NSUInteger)index, [self length] - (NSUInteger)index);
  NSRange range =
      [self rangeOfString:s options:NSLiteralSearch range:searchRange];
  return range.location == NSNotFound ? -1 : (int)range.location;
}
...
- (NSInteger)indexOf:(int)ch fromIndex:(NSInteger)index {
  unichar c = (unichar)ch;
  NSString *s = [NSString stringWithCharacters:&c length:1];
  return [self indexOfString:s fromIndex:index];
}
...
- (NSString *)stringByReplacingRegexPattern:(NSString *)regex
  ...
  return [self stringByReplacingRegexPattern:regex
...
- (NSString *)stringByReplacingRegexPattern:(NSString *)regex
  ...
  NSString *retVal =
      [pattern stringByReplacingMatchesInString:self
                                        options:0
...
- (NSArray *)stringsByExtractingGroupsUsingRegexPattern:(NSString *)regex
  ...
  [pattern enumerateMatchesInString:self
                         ...
                         usingBlock:^(NSTextCheckingResult *result,
                             ...
                             // Note, we only want to return the things in
                             // parens, so we're skipping index 0 intentionally
                             for (int i = 1; i < [result numberOfRanges]; i++) {
                               NSString *matchedString = [self
...
- (NSArray *)stringsByExtractingGroupsUsingRegexPattern:(NSString *)regex {
  return [self stringsByExtractingGroupsUsingRegexPattern:regex
                                          caseInsensitive:NO
                                           treatAsOneLine:NO];
}
...
- (BOOL)matchesPatternRegexPattern:(NSString *)regex {
  return [self matchesPatternRegexPattern:regex
                          caseInsensitive:NO
                           treatAsOneLine:NO];
}
