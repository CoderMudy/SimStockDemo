...
- (id)initWithTstockID:(NSString *)stockID
  ...
  if (self) {
    ...
    self.stockID = stockID;
    self.sourceID = sourceid;
    self.midTweetObject = tweetObject;
    // 发布聊股 回调函数
    OnReturnObjectCallback = callback;
  }
...
- (void)viewDidLoad {
  [super viewDidLoad];
  [_topToolBar setTitleCenter:@"回复评论"];
  [self setup];
}
///重写父类函数
- (void)setup {
  self.messageDisplayView =
      [[YLTextView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, WIDTH_OF_SCREEN,
                                                   self.messageToolView.frame.origin.y - (self.view.bottom - self.clientView.bottom))
                               andTitle:NO
                                andType:YLReviewTextView];
...
- (void)rightButtonPress {
  ...
  if (([contentStr length] > 0 && ![SimuUtil isBlankString:contentStr]) || shareImage != nil) {
    ...
    NSString *headerStr =
        [NSString stringWithFormat:@"回复<atuser uid=\"%@\" nick=\"%@\"/> : %@", self.midTweetObject.userListItem.userId,
                                   _midTweetObject.userListItem.nickName, contentStr ? contentStr : @""];
    NSString *resultStr = [self.messageDisplayView getXMLfromstring:headerStr];

    [self Edit_StockComments:resultStr andImage:shareImage];
  } else {
    [NewShowLabel setMessageContent:@"请输入回复评论内容"];
  }
}
...
- (void)Edit_StockComments:(NSString *)string andImage:(UIImage *)shareImage {
  ...
  ///返回对象
  TweetListItem *tweetlistItem = [[TweetListItem alloc] initWithReviewStocktalkId:self.stockID
                                                                       andContext:string
                                                                         andImage:shareImage
                                                                      andReviewID:self.sourceID
                                                                   andTweetObject:self.midTweetObject];
