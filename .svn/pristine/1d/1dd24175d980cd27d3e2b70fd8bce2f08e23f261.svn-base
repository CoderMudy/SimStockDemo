...
- (void)DeleteThisImage:(NSInteger)ThisImageIndex {
  ...
  NSLog(@"_currentPhotoIndex---%ld", (long)_currentPhotoIndex);
...
- (void)createScrollView {
  ...
  _photoScrollView.autoresizingMask =
  ...
  _photoScrollView.contentSize =
      CGSizeMake(frame.size.width * _photos.count, 0);
  [self.view addSubview:_photoScrollView];
  _photoScrollView.contentOffset =
      CGPointMake(_currentPhotoIndex * frame.size.width, 0);
...
- (void)setCurrentPhotoIndex:(NSUInteger)currentPhotoIndex {
  ...
  if ([self isViewLoaded]) {
    _photoScrollView.contentOffset =
        CGPointMake(_currentPhotoIndex * _photoScrollView.frame.size.width, 0);
...
- (void)showPhotos {
  ...
  NSInteger firstIndex = (NSInteger)floorf((CGRectGetMinX(visibleBounds) + kPadding * 2) /
                               CGRectGetWidth(visibleBounds));
  NSInteger lastIndex =
      (NSInteger)floorf((CGRectGetMaxX(visibleBounds) - kPadding * 2 - 1) /
                  CGRectGetWidth(visibleBounds));
  if (firstIndex < 0)
    firstIndex = 0;
...
//  photoView.frame = photoViewFrame;
  photoView.frame = CGRectMake(0, 0, WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN);
...
- (void)updateTollbarState {
  _currentPhotoIndex =
      _photoScrollView.contentOffset.x / _photoScrollView.frame.size.width;
  _toolbar.currentPhotoIndex = _currentPhotoIndex;
}
...
//#pragma mark - Yuemeng 横屏支持（必须工程开启才行）
