...
- (void)initializeResource {
  ...
  NSString *oldCachesDirectory = [[[cachesDirectory
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:oldCachesDirectory]) {
    [[NSFileManager defaultManager] removeItemAtPath:oldCachesDirectory
                                               error:NULL];
  }
  ...
  NSDictionary *dataMap =
      (NSDictionary *)[self cachedObjectForKey:kCacheKeyForUnicode2Pinyin];
  if (dataMap) {
    self->_unicodeToHanyuPinyinTable = dataMap;
  } else {
    NSString *resourceName =
        [[NSBundle mainBundle] pathForResource:@"unicode_to_hanyu_pinyin"
                                        ofType:@"txt"];
    NSString *dictionaryText =
        [NSString stringWithContentsOfFile:resourceName
    ...
    [self cacheObjec:self->_unicodeToHanyuPinyinTable
              forKey:kCacheKeyForUnicode2Pinyin];
  }
}
...
- (NSArray *)getHanyuPinyinStringArrayWithChar:(unichar)ch {
  ...
  if (nil != pinyinRecord) {
    ...
    NSString *stripedString = [pinyinRecord
        substringWithRange:NSMakeRange(rangeOfLeftBracket.location +
                                           rangeOfLeftBracket.length,
                                       rangeOfRightBracket.location -
                                           rangeOfLeftBracket.location -
                                           rangeOfLeftBracket.length)];
...
- (BOOL)isValidRecordWithNSString:(NSString *)record {
  ...
  if ((nil != record) && ![record isEqual:noneStr] &&
      [record hasPrefix:LEFT_BRACKET] && [record hasSuffix:RIGHT_BRACKET]) {
    return YES;
  } else
    return NO;
}
...
- (NSString *)getHanyuPinyinRecordFromCharWithChar:(unichar)ch {
  ...
  NSString *codepointHexStr =
      [[NSString stringWithFormat:@"%x", codePointOfChar] uppercaseString];
  NSString *foundRecord =
      self->_unicodeToHanyuPinyinTable[codepointHexStr];
