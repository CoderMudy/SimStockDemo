...
@interface UIColor (STSegmentedControl)
...
- (id)init {
  ...
  if (self) {
    ...
    self.frame =
        CGRectMake(10.0, 0.0, kURBDefaultSize.width, kURBDefaultSize.height);
...
- (id)initWithIcons:(NSArray *)icons {
  self = [self init];
  if (self) {
    [icons enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx,
                                        BOOL *stop) {
...
- (id)initWithTitles:(NSArray *)titles icons:(NSArray *)icons {
  ...
  self = [self init];
  if (self) {
    [titles enumerateObjectsUsingBlock:^(NSString *title, NSUInteger idx,
                                         BOOL *stop) {
...
- (void)insertSegmentWithTitle:(NSString *)title
  ...
  [segmentView setImage:[image imageTintedWithColor:self.imageColor]
               forState:UIControlStateNormal];
  [segmentView setImage:[image imageTintedWithColor:self.selectedImageColor]
  ...
  if (self.segmentTextAttributes) {
    [segmentView setTextAttributes:self.segmentTextAttributes
                          forState:UIControlStateNormal];
  }
  ...
  // set initial frame for segment, which will be adjusted later when we
  // actually lay everything out
  // we just need some size for proper height of the background images that are
  // drawn
  CGRect segmentRect = CGRectMake(
      0, 0, MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)),
      MIN(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds)));
  segmentView.frame = CGRectInset(segmentRect, self.segmentEdgeInsets.top,
                                  self.segmentEdgeInsets.left);
...
- (void)insertSegmentWithImage:(UIImage *)image
                       atIndex:(NSUInteger)segment
                      animated:(BOOL)animated {
  [self insertSegmentWithTitle:nil
                         image:image
                       atIndex:segment
                      animated:animated];
}
...
- (void)removeSegmentAtIndex:(NSUInteger)segment animated:(BOOL)animated {
  ...
  if (self.selectedSegmentIndex >= 0) {
    ...
    if (self.items.count == 1) {
      ...
      self.selectedSegmentIndex =
          [self firstEnabledSegmentIndexNearIndex:self.selectedSegmentIndex];
      changed = YES;
    } else if (self.selectedSegmentIndex > segment) {
      self.selectedSegmentIndex =
          [self firstEnabledSegmentIndexNearIndex:self.selectedSegmentIndex];
...
- (void)setImage:(UIImage *)image forSegmentAtIndex:(NSUInteger)segment {
  [[self segmentAtIndex:segment]
      setImage:[image imageTintedWithColor:self.imageColor]
      forState:UIControlStateNormal];
  [[self segmentAtIndex:segment]
      setImage:[image imageTintedWithColor:self.selectedImageColor]
      forState:UIControlStateSelected];
}
...
- (void)setSelectedSegmentIndex:(NSInteger)selectedSegmentIndex {
  if (_selectedSegmentIndex != selectedSegmentIndex) {
    ...
    if (_selectedSegmentIndex >= 0)
      ((URBSegmentView *)[self segmentAtIndex:_selectedSegmentIndex]).selected =
          NO;

    ((URBSegmentView *)[self segmentAtIndex:selectedSegmentIndex]).selected =
        YES;
...
- (void)setSegmentViewLayout:(URBSegmentViewLayout)segmentViewLayout {
  if (segmentViewLayout != _segmentViewLayout) {
    [self.items enumerateObjectsUsingBlock:^(URBSegmentView *segmentView,
...
- (void)setImageColor:(UIColor *)imageColor forState:(UIControlState)state {
  [self.items enumerateObjectsUsingBlock:^(URBSegmentView *segmentView,
    ...
    UIColor *color = (state == UIControlStateSelected) ? self.selectedImageColor
                                                       : self.imageColor;
...
- (UIImage *)defaultBackgroundImage {
  ...
  // gradients
  NSArray *baseGradientColors = @[
    (id)baseGradientTopColor.CGColor,
    (id)baseGradientBottomColor.CGColor
  ];
  CGFloat baseGradientLocations[] = {0, 1};
  CGGradientRef baseGradient = CGGradientCreateWithColors(
      colorSpace, (__bridge CFArrayRef)baseGradientColors,
      baseGradientLocations);
  ...
  {
    // base path
    CGRect backgroundBaseRect =
        CGRectMake(CGRectGetMinX(frame) + stroke / 2.0,
                   CGRectGetMinY(frame) + stroke / 2.0, size.width - stroke,
                   size.height - stroke);
    UIBezierPath *backgroundBasePath =
        [UIBezierPath bezierPathWithRoundedRect:backgroundBaseRect
                                   cornerRadius:radius];
    CGContextSaveGState(context);
    [backgroundBasePath addClip];
    CGContextDrawLinearGradient(context, baseGradient,
                                CGPointMake(CGRectGetMidX(backgroundBaseRect),
                                            CGRectGetMinY(backgroundBaseRect)),
                                CGPointMake(CGRectGetMidX(backgroundBaseRect),
                                            CGRectGetMaxY(backgroundBaseRect)),
                                0);
    CGContextRestoreGState(context);

    // inner shadow
    CGRect backgroundBaseBorderRect =
        CGRectInset([backgroundBasePath bounds], -baseInnerShadowBlurRadius,
                    -baseInnerShadowBlurRadius);
    backgroundBaseBorderRect =
        CGRectOffset(backgroundBaseBorderRect, -baseInnerShadowOffset.width,
                     -baseInnerShadowOffset.height);
    backgroundBaseBorderRect = CGRectInset(
        CGRectUnion(backgroundBaseBorderRect, [backgroundBasePath bounds]), -1,
        -1);

    UIBezierPath *backgroundBaseNegativePath =
        [UIBezierPath bezierPathWithRect:backgroundBaseBorderRect];
    [backgroundBaseNegativePath appendPath:backgroundBasePath];
    backgroundBaseNegativePath.usesEvenOddFillRule = YES;

    CGContextSaveGState(context);
    {
      CGFloat xOffset = baseInnerShadowOffset.width +
                        round(backgroundBaseBorderRect.size.width);
      CGFloat yOffset = baseInnerShadowOffset.height;
      CGContextSetShadowWithColor(
          context, CGSizeMake(xOffset + copysign(0.1, xOffset),
                              yOffset + copysign(0.1, yOffset)),
          baseInnerShadowBlurRadius, baseInnerShadow.CGColor);

      [backgroundBasePath addClip];
      CGAffineTransform transform = CGAffineTransformMakeTranslation(
          -round(backgroundBaseBorderRect.size.width), 0);
  ...
  return [image resizableImageWithCapInsets:UIEdgeInsetsMake(topCap, leftCap,
                                                             topCap, leftCap)];
}
...
- (NSInteger)firstEnabledSegmentIndexNearIndex:(NSUInteger)index {
  for (NSUInteger i = index; i < self.items.count; i++) {
    if (((URBSegmentView *)self.items[i]).enabled) {
      return i;
    }
  }
...
- (void)layoutSegments {
  ...
  // calculate width of each segment based on number of items and total
  // available width
  CGRect segmentRect = CGRectInset(self.bounds, self.segmentEdgeInsets.top,
                                   self.segmentEdgeInsets.left);
  CGSize totalSize = segmentRect.size;
  CGSize segmentSize =
      (self.layoutOrientation == URBSegmentedControlOrientationVertical)
          ? CGSizeMake(totalSize.width, totalSize.height / self.items.count)
          : CGSizeMake(totalSize.width / self.items.count, totalSize.height);

  [self.items enumerateObjectsUsingBlock:^(URBSegmentView *item, NSUInteger idx,
                                           BOOL *stop) {
    if (self.layoutOrientation == URBSegmentedControlOrientationVertical) {
      item.frame =
          CGRectMake(CGRectGetMinX(segmentRect),
                     CGRectGetMinY(segmentRect) + segmentSize.height * idx,
                     segmentSize.width, segmentSize.height);
    } else {
      item.frame = CGRectMake(
          CGRectGetMinX(segmentRect) + segmentSize.width * idx,
          CGRectGetMinY(segmentRect), segmentSize.width, segmentSize.height);
    }
    [item setNeedsLayout];
  }];
}
...
+ (void)initialize {
  ...
  [appearance setTitleColor:[UIColor lightGrayColor]
                   forState:UIControlStateNormal];
  [appearance setTitleShadowColor:[UIColor blackColor]
                         forState:UIControlStateNormal];

  [appearance setTitleColor:[UIColor whiteColor]
                   forState:UIControlStateSelected];
  [appearance setTitleShadowColor:[UIColor blackColor]
                         forState:UIControlStateSelected];
}
...
- (id)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    self.titleLabel.font =
        [UIFont fontWithName:@"HelveticaNeue-CondensedBold" size:14.0];
...
- (void)setTextAttributes:(NSDictionary *)textAttributes
  ...
  UIColor *textShadowColor = textAttributes[UITextAttributeTextShadowColor];
  NSValue *shadowOffsetValue = textAttributes[UITextAttributeTextShadowOffset];
  ...
  if (shadowOffsetValue) {
    ...
    self.titleLabel.shadowOffset =
        CGSizeMake(shadowOffset.horizontal, shadowOffset.vertical);
  }
}
...
- (void)layoutSubviews {
  ...
  // automatically reset title and image insets if this segment is below a
  // certain size to prevent
  ...
  // don't draw background images until we have a valid size so that our
  // vertical gradients display properly.
  if (!_hasDrawnImages) {
    _hasDrawnImages = YES;
  ...
  if (self.viewLayout == URBSegmentViewLayoutVertical) {
    // only adjust layout if we have both a title and image, otherwise the
    ...
    if (self.titleLabel.text.length > 0 && self.imageView.image) {
      ...
      self.imageView.frame =
          UIEdgeInsetsInsetRect(imageRect, self.imageEdgeInsets);

      // title should start at bottom of content area with bottom inset
      CGRect titleFrame = CGRectMake(CGRectGetMinX(frame), 0,
                                     CGRectGetWidth(frame), titleSize.height);
      titleFrame.origin.y =
          CGRectGetMaxY(frame) - self.titleEdgeInsets.bottom - titleSize.height;
...
- (void)updateBackgrounds {
  [self setBackgroundImage:[self normalBackgroundImage]
                  forState:UIControlStateNormal];
  [self setBackgroundImage:[self selectedBackgroundImage]
                  forState:UIControlStateSelected];
}
...
- (UIImage *)selectedBackgroundImage {
  ...
  // colors
  UIColor *segmentGradientTopColor =
      [self.imageBackgroundColor adjustBrightness:1.2];
  UIColor *segmentGradientBottomColor =
      [self.imageBackgroundColor adjustBrightness:0.8];
  UIColor *segmentStrokeColor =
      [self.imageBackgroundColor adjustBrightness:0.6];
  UIColor *segmentHighlight = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.7];

  // gradients
  NSArray *segmentGradientColors = @[
    (id)segmentGradientTopColor.CGColor,
    (id)segmentGradientBottomColor.CGColor
  ];
  CGFloat segmentGradientLocations[] = {0, 1};
  CGGradientRef segmentGradient = CGGradientCreateWithColors(
      colorSpace, (__bridge CFArrayRef)segmentGradientColors,
      segmentGradientLocations);
  ...
  {
    CGContextSaveGState(context);
    CGContextSetShadowWithColor(context, segmentShadowOffset,
                                segmentShadowBlurRadius, segmentShadow.CGColor);
    CGContextBeginTransparencyLayer(context, NULL);

    // outer path
    UIBezierPath *segmentBaseOuterPath = [UIBezierPath
        bezierPathWithRoundedRect:CGRectMake(CGRectGetMinX(frame),
                                             CGRectGetMinY(frame),
    ...
    // inner path
    CGRect segmentBaseRect =
        CGRectMake(CGRectGetMinX(frame) + stroke, CGRectGetMinY(frame) + stroke,
                   CGRectGetWidth(frame) - stroke * 2.0,
                   CGRectGetHeight(frame) - stroke * 2.0);
    UIBezierPath *segmentBasePath =
        [UIBezierPath bezierPathWithRoundedRect:segmentBaseRect
                                   cornerRadius:radius];
    CGContextSaveGState(context);
    [segmentBasePath addClip];
    CGContextDrawLinearGradient(context, segmentGradient,
                                CGPointMake(CGRectGetMidX(segmentBaseRect),
                                            CGRectGetMinY(segmentBaseRect)),
                                CGPointMake(CGRectGetMidX(segmentBaseRect),
                                            CGRectGetMaxY(segmentBaseRect)),
                                0);
    CGContextRestoreGState(context);

    // inner shadow
    CGRect segmentBaseBorderRect =
        CGRectInset([segmentBasePath bounds], -segmentHighlightBlurRadius,
                    -segmentHighlightBlurRadius);
    segmentBaseBorderRect =
        CGRectOffset(segmentBaseBorderRect, -segmentHighlightOffset.width,
                     -segmentHighlightOffset.height);
    segmentBaseBorderRect = CGRectInset(
        CGRectUnion(segmentBaseBorderRect, [segmentBasePath bounds]), -1, -1);

    UIBezierPath *segmentBaseNegativePath =
        [UIBezierPath bezierPathWithRect:segmentBaseBorderRect];
    [segmentBaseNegativePath appendPath:segmentBasePath];
    segmentBaseNegativePath.usesEvenOddFillRule = YES;

    CGContextSaveGState(context);
    {
      CGFloat xOffset = segmentHighlightOffset.width +
                        round(segmentBaseBorderRect.size.width);
      CGFloat yOffset = segmentHighlightOffset.height;
      CGContextSetShadowWithColor(
          context, CGSizeMake(xOffset + copysign(0.1, xOffset),
                              yOffset + copysign(0.1, yOffset)),
          segmentHighlightBlurRadius, segmentHighlight.CGColor);

      [segmentBasePath addClip];
      CGAffineTransform transform = CGAffineTransformMakeTranslation(
          -round(segmentBaseBorderRect.size.width), 0);
...
@implementation UIImage (STSegmentedControl)

/**
 UIImage tint category methods originally developed by Matt Gemmell and released
 under the BSD License: http://mattgemmell.com/license/
...
@implementation UIColor (STSegmentedControl)
