...
// Byte* ds : 8个字节
// int index: 起始索引
static int64_t readInt64(const Byte *ds, NSInteger index) {
  int64_t temp;
  Byte mb[8];
  mb[0] = ds[index++];
  mb[1] = ds[index++];
...
// Byte* ds : 8个字节
// int index: 起始索引
static double readDouble(const Byte *ds, NSInteger index) {
...
// Byte* ds : 4个字节
// int index: 起始索引
static int readInt(const Byte *ds, NSInteger index) {
  int temp;
  Byte mb[4];
  mb[0] = ds[index++];
  mb[1] = ds[index++];
...
// Byte *ds : 4个字节
// int index: 起始索引
static float readFloat(const Byte *ds, NSInteger index) {
...
//从字节数中读取short数据
// Byte *ds : 2个字节
// NSInteger index: 起始索引
static short readshort(const Byte *ds, NSInteger index) {
  short temp;
  Byte mb[2];
  mb[0] = ds[index++];
  mb[1] = ds[index++];
  temp = (((NSInteger)mb[0] & 0xff) << 8) | ((NSInteger)mb[1] & 0xff);
  return temp;
}
...
// Byte *ds : 2个字节
// NSInteger index: 起始索引
static short readByte(const Byte *ds, NSInteger index) {
  return ((NSInteger)ds[index] & 0xff);
}
...
// Byte *ds : 不定字节数
// NSNumber** : 起始索引
static NSInteger readCompressInt(const Byte *ds, NSNumber **index) {
  NSInteger start_index = [(*index)intValue];
  NSInteger val = 0;
  Byte b[1];
  NSInteger ind = 0;
...
// Byte *ds : 4个节数
// index: 起始索引
static int64_t readCompressDateTime(const Byte *ds, NSInteger index) {
  int temp;
  Byte mb[4];
  mb[0] = ds[index++];
  mb[1] = ds[index++];
...
- (float)readFloatAt:(NSInteger)startIndex {
  return readFloat([self bytes], startIndex);
}

- (double)readDoubleAt:(NSInteger)startIndex {
  return readDouble([self bytes], startIndex);
}

- (int)readIntAt:(NSInteger)startIndex {
  return readInt([self bytes], startIndex);
}
...
- (short)readByteAt:(NSInteger)startIndex {
  return readByte([self bytes], startIndex);
}

- (int64_t)readInt64At:(NSInteger)startIndex {
  return readInt64([self bytes], startIndex);
}
...
- (int64_t)readCompressDateTimeAt:(NSInteger)startIndex {
  return readCompressDateTime([self bytes], startIndex);
}
