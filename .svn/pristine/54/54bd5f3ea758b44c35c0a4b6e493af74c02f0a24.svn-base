...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    //添加旋转手势 added by Yuemeng 2015.1.21
    UIRotationGestureRecognizer *rotaGesture =
        [[UIRotationGestureRecognizer alloc] initWithTarget:self action:@selector(changeRotation:)];
    rotaGesture.delegate = self;
    [self addGestureRecognizer:rotaGesture];
  }
  return self;
}
#pragma mark - 手势代理方法，同时支持多个手势
...
- (void)adjustFrame {
  ...
  CGSize boundsSize = CGSizeMake(WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN);
  ...
  self.contentSize = CGSizeMake(WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN);
...
///旋转处理
- (void)changeRotation:(UIRotationGestureRecognizer *)rotaGesture {
  //旋转处理
  if (rotaGesture.state == UIGestureRecognizerStateBegan || rotaGesture.state == UIGestureRecognizerStateChanged) {
    self.transform = CGAffineTransformRotate(self.transform, rotaGesture.rotation);
    [rotaGesture setRotation:0];
  } else if (rotaGesture.state == UIGestureRecognizerStateEnded) {
    CGAffineTransform initTran = CGAffineTransformMakeTranslation(0, 0);
    [UIView animateWithDuration:0.25
                     animations:^{
                       // X型区域：
                       //上面
                       if (0.5f <= self.transform.a && self.transform.a <= 1.0f) {
                         self.transform = CGAffineTransformRotate(initTran, 0.0f);
                         self.contentSize = CGSizeMake(WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN);
                         //下面
                       } else if (-1.0f < self.transform.a && self.transform.a < -0.5f) {
                         self.transform = CGAffineTransformRotate(initTran, M_PI);
                         self.contentSize = CGSizeMake(WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN);
                         //左或右
                       } else if (-0.5f < self.transform.a && self.transform.a < 0.5f) {
                         //右
                         if (self.transform.b >= 0) {
                           self.transform = CGAffineTransformRotate(initTran, M_PI / 2);
                           self.contentSize = CGSizeMake(HEIGHT_OF_SCREEN, WIDTH_OF_SCREEN);
                           //左
                         } else {
                           self.transform = CGAffineTransformRotate(initTran, -M_PI / 2);
                           self.contentSize = CGSizeMake(HEIGHT_OF_SCREEN, WIDTH_OF_SCREEN);
