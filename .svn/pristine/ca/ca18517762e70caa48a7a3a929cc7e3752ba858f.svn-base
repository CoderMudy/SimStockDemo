...
//  self.transform = CGAffineTransformScale(self.transform, 0.8, 0.8);
  ...
  if ([_viewArr11 count] <= 4 && _array == _viewArr11) {
    if (CGRectEqualToRect(stateFrame, self.frame)|| point.y > self.moreChannelsLabel.top)
    {
      //提示语 动画
      [NewShowLabel setMessageContent:@"最少保留4个栏目"];
    }
...
- (void)touchesMoved:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  if (self.isEditable) {
    ...
    if (!CGRectContainsPoint([[_viewArr11 objectAtIndex:0] frame],
