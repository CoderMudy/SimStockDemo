...
@implementation MatchChatStockPageTVAdapter
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  cellHeight += [MatchChatStockPageTVAdapter getTitleAndContentAndImageHeightWithWeibo:item
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  /// 绑定聊股标题、聊股内容、聊股图片数据
  height += [MatchChatStockPageTVAdapter bindTittleAndContentAndContentImageAtCell:cell
                                                                      andIndexPath:indexPath
                                                                  andTopViewBottom:height
                                                                      andTableView:tableView
                                                                andHasUserNameView:NO];

  height += [MatchChatStockPageTVAdapter bindRPContentAndRPComtentImageAtCell:cell
                                                                 andIndexPath:indexPath
                                                                 andTableView:tableView
                                                             andTopViewBottom:height
                                                           andHasUserNameView:NO];
  ...
  __weak MatchChatStockPageTVAdapter *weakSelf = self;
  /// 重写取消收藏回调函数，屏蔽默认实现
...
- (NSDictionary *)getRequestParamertsWithRefreshType:(RefreshType)refreshType {
  ...
  return @{ @"title" : matchTitle, @"fromId" : [fromId stringValue], @"reqNum" : pageSize };
}
...
- (void)requestDataListWithRefreshType:(RefreshType)refreshType
  ...
  [TweetList requestGetSubjectTweetListWithParams:dic withCallback:callback];
}

/** 返回表格数据适配器，此方法子类必须实现 */
- (BaseTableAdapter *)getBaseTableAdaperWithDataList:(DataArray *)dataList {
  if (_tableAdapter == nil) {
    _tableAdapter = [[MatchChatStockPageTVAdapter alloc] initWithTableViewController:self
    ...
    /// 删除聊股时的回调
    __weak MatchChatStockPageTableVC *weakSelf = self;
    MatchChatStockPageTVAdapter *adapter = (MatchChatStockPageTVAdapter *)_tableAdapter;
