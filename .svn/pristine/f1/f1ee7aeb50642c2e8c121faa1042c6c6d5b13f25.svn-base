...
- (void)drawRect:(CGRect)rect {
  ...
  CGColorRef onColorCG = onColor
                             ? onColor.CGColor
                             : [UIColor colorWithWhite:1.0f alpha:1.0f].CGColor;
  CGColorRef offColorCG =
      offColor ? offColor.CGColor
               : [UIColor colorWithWhite:0.7f alpha:0.5f].CGColor;
  ...
  for (int i = 0; i < numberOfPages; i++) {
    ...
    if (i == currentPage) {
      if (type == DDPageControlTypeOnFullOffFull ||
          type == DDPageControlTypeOnFullOffEmpty) {
        CGContextSetFillColorWithColor(context, onColorCG);
        CGContextFillEllipseInRect(context, CGRectInset(dotRect, 0.f, 0.f));
      } else {
        CGContextSetStrokeColorWithColor(context, onColorCG);
        CGContextStrokeEllipseInRect(context, dotRect);
      }
    } else {
      if (type == DDPageControlTypeOnEmptyOffEmpty ||
          type == DDPageControlTypeOnFullOffEmpty) {
...
- (void)setNumberOfPages:(NSInteger)numOfPages {
  ...
  // depending on the user preferences, we hide the page control with a single
  // element
...
- (void)setFrame:(CGRect)aFrame {
  // we do not allow the caller to modify the size struct in the frame so we
  // compute it
...
- (void)setBounds:(CGRect)aBounds {
  // we do not allow the caller to modify the size struct in the bounds so we
  // compute it
...
- (void)updateCurrentPageDisplay {
  ...
  // in case it is YES, we redraw the view (that will update the page control to
  // the correct page)
  [self setNeedsDisplay];
}
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount {
  ...
  return CGSizeMake(pageCount * diameter + (pageCount - 1) * space + 18.0f,
                    MAX(21.0f, diameter + 4.0f));
}
