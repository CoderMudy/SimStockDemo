...
define("project/scripts/account/signProtocol",function(require, exports, module){ 
	...
	function bindPageEvent()
	{
		...
		/* 绑定签署协议*/
		appUtils.bindEvent($(_pageId+" .rule_check .icon_check"),function(){
	...
	function initPage()
	{
		...
		{
			$(_pageId+" .fix_bot .ct_btn a").html("继续开户 >");  // 开户
		}
	}
	...
	function initSelcet()
	{
		...
		{
			$(_pageId+" .radio_list ul li a").addClass("checked");
			getAgreement();  // 获取协议
		}
		// 转户需要查询原有的股东账户
		...
		service.queryProtocolList({"category_no":"1"},function(data){
			if(data.error_no == 0 && data.results.length != 0)
			{
				...
				for(var i=0;i<results.length;i++)
				{
					...
					allProtocols += "<li><a href=\"javascript:void(0);\" protocol-id=\""+results[i].econtract_no+"\" id=\"protocol0"+i+"\">《"+
	...
	function getSignProtocl()
	{
		...
		// 检查是否勾选签署协议
		if($(_pageId+" .rule_check a").hasClass("checked"))
		{
			layerUtils.iLoading(true);  // 开启等待层。。。
