...
#import "LogonSlipViewController.h"
#import "SimuAction.h"
...
@implementation LogonSlipViewController
...
- (void)createBaseView {
  ...
  //登录后的页面头部
  _loginSuccessView =
      [[UIView alloc] initWithFrame:CGRectOffset(self.view.bounds, -30, 0)];
  _loginSuccessView.backgroundColor =
      [Globle colorFromHexRGB:Color_White alpha:0.15];
...
- (void)drawBackgroundColor {
  ...
  _backImageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, kHeadViewHeight + HEIGHT_0F_STATUSBAR,
                               SIDE_SLIP_WIDTH,
                               self.view.frame.size.height - kHeadViewHeight -
                                   HEIGHT_0F_STATUSBAR)];
  ...
  CGFloat colors[] = {
      29.0 / 255.0, 39.0 / 255.0, 40.0 / 255.0, 1, 48.0 / 255.0, 50.0 / 255.0,
      60.0 / 255.0, 1, 51.0 / 255.0, 53.0 / 255.0, 58.0 / 255.0, 1,
      38.0 / 255.0, 49.0 / 255.0, 58.0 / 255.0, 1, 32.0 / 255.0, 42.0 / 255.0,
      44.0 / 255.0, 1,
  };
  ...
  CGContextDrawLinearGradient(
      context, gradient, CGPointMake(0, kHeadViewHeight),
      CGPointMake(SIDE_SLIP_WIDTH,
                  self.view.frame.size.height - kHeadViewHeight),
...
- (void)createAllBaseControllerView {
  //创建用户背景头像图片
  //创建用户头像图片
  _userHeadImageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(12, 9 + HEIGHT_0F_STATUSBAR, 56, 56)];
  ...
  //总盈利名称标签
  UILabel *totalRateLabel = [[UILabel alloc]
      initWithFrame:CGRectMake(80, 14 + HEIGHT_0F_STATUSBAR, 120, 12)];
  ...
  //总盈利数值
  _userFundLabel = [[UILabel alloc]
      initWithFrame:CGRectMake(80, 35 + HEIGHT_0F_STATUSBAR, 140, 21)];
  ...
  //我的主页标签
  UILabel *myHomeLabel = [[UILabel alloc]
      initWithFrame:CGRectMake(SIDE_SLIP_WIDTH - 44 - 4,
                               10 + HEIGHT_0F_STATUSBAR, 44, 48)];
  ...
  UIImageView *ArrowImageView = [[UIImageView alloc]
      initWithImage:[UIImage imageNamed:@"SST_Main_arrow.png"]];
  ArrowImageView.frame =
      CGRectMake(SIDE_SLIP_WIDTH - 10 - 10, 26 + HEIGHT_0F_STATUSBAR, 10, 18);
  ...
  for (int i = 0; i < 4; ++i) {
    UIView *lineView = [[UIView alloc]
        initWithFrame:CGRectMake(i * linewidth,
                                 kHeadViewHeight - 3 + HEIGHT_0F_STATUSBAR,
                                 linewidth, 3)];
  ...
  _homeButton.frame = CGRectMake(0, HEIGHT_0F_STATUSBAR,
                                 self.view.frame.size.width, kHeadViewHeight);
  ...
  [userHeadButton setBackgroundImage:backImage
                            forState:UIControlStateHighlighted];
...
- (void)createTableView {
  _tableView = [[UITableView alloc]
      initWithFrame:CGRectMake(0, kHeadViewHeight + HEIGHT_0F_STATUSBAR,
                               ...
                               kCellHeight * _menuNameArray.count)];
...
- (void)createSettingAndMallButton {
  ...
  [_settingBtn setImage:[UIImage imageNamed:@"设置"]
  ...
  _mallBtn.alpha = 0.8;
  _mallBtn.frame =
      CGRectMake(SIDE_SLIP_WIDTH - 10 - 65, self.view.bottom - 36, 65, 36);
  _mallBtn.backgroundColor = [UIColor clearColor];
  [_mallBtn setImage:[UIImage imageNamed:@"商店"]
            forState:UIControlStateNormal];
  ...
  [_mallBtn setOnButtonPressedHandler:^{
    ...
    NetShoppingMallBaseViewController *mallVC =
        [[NetShoppingMallBaseViewController alloc]
            initWithPageType:Mall_Buy_Diamond_Mode];
...
- (void)registerMessageSeverse {
  ...
  __weak LogonSlipViewController *weakSelf = self;
  ...
  //左侧栏显示总盈利的通知
  [[NSNotificationCenter defaultCenter]
      addObserverForName:AccountTotalProfitNotification
                  object:nil
...
- (void)failedPurchase:(EBPurchase *)ebp
  ...
  UIAlertView *failedAlert =
      [[UIAlertView alloc] initWithTitle:@"交易停止"
                                 message:@"交易失败，请重试！ "
                                delegate:nil
...
- (void)incompleteRestore:(EBPurchase *)ebp {
  ...
  UIAlertView *restoreAlert = [[UIAlertView alloc]
          initWithTitle:@"Restore Issue"
                message:@"A prior purchase transaction could not be found. To "
                @"restore the purchased product, tap the Buy button. "
                @"Paid customers will NOT be charged again, but the "
...
- (void)failedRestore:(EBPurchase *)ebp
  ...
  UIAlertView *failedAlert =
      [[UIAlertView alloc] initWithTitle:@"交易中止"
                                 message:@"交易失败，请重试！"
                                delegate:nil
                       cancelButtonTitle:@"确定"
                       otherButtonTitles:nil];
...
- (void)willPresentAlertView:(UIAlertView *)alertView {
  if (_showAlertView == alertView) {
    _actIndicatorView.center =
        CGPointMake(_showAlertView.bounds.size.width / 2.0f,
                    _showAlertView.bounds.size.height - 40.0f);
...
- (void)popLoginView {
  [AppDelegate
      pushViewControllerFromRight:[[FullScreenLogonViewController alloc] init]];
}
...
- (void)setCell:(UITableViewCell *)cell andIndex:(NSInteger)index {
  //分割线
  UIView *viewline =
      [[UIView alloc] initWithFrame:CGRectMake(15, kCellHeight - 0.5,
                                               SIDE_SLIP_WIDTH - 30, 0.5)];
  viewline.backgroundColor =
      [[Globle colorFromHexRGB:Color_White] colorWithAlphaComponent:0.08];
  [cell addSubview:viewline];

  //签到送金币或其他标签
  UILabel *menuLabel = [[UILabel alloc]
      initWithFrame:CGRectMake((index == 0 ? 44 : 14),
                               viewline.frame.origin.y - 26.5f, 140, 14)];
  menuLabel.backgroundColor = [UIColor clearColor];
  menuLabel.text = _menuNameArray[index];
  menuLabel.textColor =
      [[Globle colorFromHexRGB:Color_White] colorWithAlphaComponent:0.8];
  ...
  if (index == 0) {
    ...
    UIImageView *imageView = [[UIImageView alloc]
        initWithFrame:CGRectMake(14, (viewline.frame.origin.y - 24) / 2, 23,
                                 24)];
    ...
    UIImageView *imageView = [[UIImageView alloc]
        initWithFrame:CGRectMake(SIDE_SLIP_WIDTH - image.size.width - 15,
    ...
    UIImageView *imageView = [[UIImageView alloc]
        initWithFrame:CGRectMake(SIDE_SLIP_WIDTH - image.size.width - 15 + .75f,
    ...
    UIImageView *imageView = [[UIImageView alloc]
        initWithFrame:CGRectMake(SIDE_SLIP_WIDTH - image.size.width - 15 + 2,
                                 viewline.frame.origin.y - 25, 21, 20)];
    imageView.image = image;
    [cell addSubview:imageView];
  } else {
    //进度条线
    SimuProgressView *progrem = [[SimuProgressView alloc]
        initWithFrame:CGRectMake(15, kCellHeight - 5.0,
                                 viewline.frame.size.width, 4.5)
             ColerStr:nil
          ProgressStr:@"1.0"];
    ...
    if (index != 0) {
      ...
      UILabel *numberLabel = [[UILabel alloc]
          initWithFrame:CGRectMake(SIDE_SLIP_WIDTH - 90,
                                   viewline.frame.origin.y - 24, 75, 16)];
...
- (void)addCollectingObserver {
  ...
  [[NSNotificationCenter defaultCenter]
   ...
   usingBlock:^(NSNotification *notif) {
     ...
     //修改本地数据
     SliderUserCounterWapper *myCounters =
     [CacheUtil loadMySliderCounter];
     int count = [myCounters.mycollect.totalNum intValue] +
     [operation intValue];
     myCounters.mycollect.totalNum =
...
- (void)refreshAttentionData {
  ...
  if (_processLabelArray && [_processLabelArray count] > 0) {
    _tempAttentionStr =
        [@([[MyAttentionInfo sharedInstance] getAttentionArray].count)
                stringValue];
  }
  //更新侧边栏数据
  [self getUserCounter];
...
- (void)pushSecondViewController:(NSInteger)cellIndex {
  ...
  case 1: {
    //我的追踪
    TrancingViewController *myTrancingVC =
        [[TrancingViewController alloc] init];
  ...
  case 2: {
    //我的关注
    MyAttentionViewController *myAttentionVC =
        [[MyAttentionViewController alloc] init];
  ...
  case 4: {
    ...
    StockPersonTransactionViewController *myDealVC =
        [[StockPersonTransactionViewController alloc]
            initWithUserID:[SimuUtil getUserID]
  ...
  case 5: {
    //我的聊股
    [AppDelegate
        pushViewControllerFromRight:[[MyChatStockViewController alloc] init]];
    break;
  }
  case 6: {
    //我的宝箱
    [AppDelegate
        pushViewControllerFromRight:[[MyChestsViewController alloc] init]];
    //纪录日志
    [[event_view_log sharedManager] addPVAndButtonEventToLog:Log_Type_Button
                                                     andCode:@"22"];
  } break;
  case 7: {
    //我的收入
    [AppDelegate
        pushViewControllerFromRight:[[MyIncomingViewController alloc] init]];
    break;
  }
  case 8: {
    //我的账户
...
- (void)getUserCounter {
  ...
  callback.onSuccess = ^(NSObject *obj) {
    LogonSlipViewController *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf bindSliderUserCounterWapper:(SliderUserCounterWapper *)obj
                                  saveToCache:YES];
    }
  };
  callback.onFailed = ^{
...
- (void)bindSliderUserCounterWapper:(SliderUserCounterWapper *)data
