...
//		require.async(protocol,function(module){
			...
			if(platform == "0")
			{
				param.op_way = param.op_source = param.infocolect_channel = 0;
			}
			// Android
			else if(platform == "1")
			{
				if(iBrowser.android){
					param.op_way = param.op_source = param.infocolect_channel = 3;  // Android 手机端
				}
				else{
					param.op_way = param.op_source = param.infocolect_channel = 2;  // Android 平板
				}
			}
			// IOS 
			else if(iBrowser.iPhone || iBrowser.iPad)
			{
				param.op_way = param.op_source = param.infocolect_channel = 4;
			}
